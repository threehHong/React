...
function Update(props) {
  ...
  return <article>
    ...
    <form onSubmit={(event) => {
      ...
      <p> 
        <input type="text" name="title" placeholder='title' value={title} onChange={(event) => { // 리액트에서 onChange는 값을 입력할 떄마다 이벤트가 발생(HTML은 입력하고 마우스를 다른 곳으로 클릭할)
...
function App() {
  ...
  4. useState를 통해 App() 재실행(컴포넌트 재실행).
  ...
  if(mode === 'WELCOME') {
    ...
    contextCotrol = <li> <a href={'/update/'+id} onClick={(event) => {
      event.preventDefault();
      setMode('UPDATE');
    }}> UPDATE </a> </li>
  } else if (mode === "CREATE") {
